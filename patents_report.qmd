...
DATA_DATE <- format(today(), "%Y_%m_%d")
...
citations <- read_csv(
  here("data", paste0(DATA_DATE, "_times_cited.csv")),
  show_col_types = FALSE
)
details <- read_csv(
  here("data", paste0(DATA_DATE, "_details.csv")),
  show_col_types = FALSE
)
status <- read_csv(
  here("data", paste0(DATA_DATE, "_status.csv")),
  show_col_types = FALSE
)
for_2020 <- read_csv(
  here("data", paste0(DATA_DATE, "_for_2020.csv")),
  show_col_types = FALSE
)
assignees <- read_csv(
  here("data", paste0(DATA_DATE, "_original_assignees.csv")),
  show_col_types = FALSE
)
```
...
group_tally <- function(data, name = "n", sort = FALSE, ...) {
  data |>
    group_by(...) |>
    tally(n = {{ name }}, sort = sort) |>
    ungroup()
}
```
...
status |>
  ...
  group_tally(name = "n", year, sort = FALSE) |>
  ggplot() +
  ...
  labs(
    title = glue(
      "Number of patents by year of publication, {MIN_YEAR} - {MAX_YEAR}"
    ),
...
for_2020_summary <- for_2020 |>
  ...
  group_tally(name = "count", for_2020_name, sort = FALSE) |>
  mutate(
    for_2020_name = str_replace(
      for_2020_name,
...
ggplot(data = for_2020_summary, aes(x = 2, y = count, fill = for_2020_name)) +
  ...
  labs(
    title = glue(
      "Fields of Research (ANZSRC2020) for patent published from {MIN_YEAR} to {MAX_YEAR}"
    ),
    caption = glue("Source: Dimensions, {DATA_DATE_TEXT}")
  ) +
  theme_void() +
...
number_of_orgs <- assignees |>
  ...
  group_tally(name = "count", n, sort = FALSE)
...
assignees |>
  ...
  group_tally(name = "n", name, sort = FALSE) |>
  filter(name != "University of Bradford") |>
  arrange(n) |>
  ...
  labs(
    title = glue(
      "Co-assignees of patents published between {MIN_YEAR} and {MAX_YEAR}"
    ),
    caption = glue("Source: Dimensions, {DATA_DATE_TEXT}")
  ) +
  theme_minimal() +
...
collaborating_org_types <- assignees |>
  ...
  group_tally(name = "n", types, sort = FALSE) |>
  filter(!is.na(types)) |>
  ggplot() +
...
collaborating_org_countries <- assignees |>
  filter(id != "grid.6268.a") |>
  group_tally(name = "n", country_name, sort = FALSE) |>
...
annotate_figure(
  fig,
  bottom = text_grob(
    glue("Source: Dimensions, {DATA_DATE_TEXT}  "),
    hjust = 1,
    x = 1,
    size = 9
  )
