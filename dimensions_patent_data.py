...
# Dimensions query
results = dsl.query(
    f"""search patents where (year in [{MIN_YEAR}:{MAX_YEAR}] and assignees = "{GRIDID}") 
...
df_original_assignees = (
    df_results.filter(["patent_id", "original_assignees"])
    .explode("original_assignees")
    .reset_index()
)

df_original_assignees = pd.concat(
    [
        df_original_assignees["patent_id"],
        pd.json_normalize(df_original_assignees["original_assignees"]),
    ],
    axis=1,
)

df_original_assignees = df_original_assignees.explode("types").drop(
    columns=["acronym", "linkout", "state_name"]
)

df_original_assignees.to_csv(
    os.path.join(DATA_DIR, "".join([TODAY, "_original_assignees.csv"])), index=False
)
...
# FoR 2020
df_for_2020 = (
    df_results.filter(["id", "category_for_2020"])
    .explode("category_for_2020")
    .reset_index()
)

df_for_2020 = pd.concat(
    [df_results["patent_id"], pd.json_normalize(df_for_2020["category_for_2020"])],
    axis=1,
)

df_for_2020 = df_for_2020.assign(
...
df_for_2020.to_csv(
    os.path.join(DATA_DIR, "".join([TODAY, "_for_2020.csv"])), index=False
)

# Citations
df_citations = df_results.filter(["patent_id", "times_cited"])
df_citations.to_csv(
    os.path.join(DATA_DIR, "".join([TODAY, "_times_cited.csv"])), index=False
)
